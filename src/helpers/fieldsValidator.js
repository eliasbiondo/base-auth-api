  ...
  if (required.includes("username")) {
    if (!username) {
      ...
      object.invalid_fields.username = {
        ...
        message:
          "The 'username' field only accepts alphanumeric characters (a-Z,0-9).",
      };
    } else {
  ...
  if (required.includes("email")) {
    if (!email) {
      ...
      object.invalid_fields.email = {
        ...
        message:
          "The 'email' field must have the following format: example@example.com",
      };
    } else {
  ...
  if (required.includes("password")) {
    if (!password) {
      ...
      object.invalid_fields.password = {
        reason: "invalid character",
        message:
          "The 'password' field only accepts lowercase characters, uppercase characters, numbers and special chars (!, @, #, $, %, ^, &, *).",
      };
    } else if (!regexExpressions.isValidPassword.test(password)) {
      object.invalid_fields.password = {
        reason: "invalid password",
        message:
          "The 'password' field must be at least one lowercase character, one uppercase character, one number and one special char (!, @, #, $, %, ^, &, *).",
      };
    } else {
  ...
  if (required.includes("full_name")) {
    ...
    if (!full_name) {
      ...
      object.invalid_fields.full_name = {
        reason: "invalid character",
        message:
          "The 'full_name' field only supports alphabetic characters (letters and spaces).",
      };
    } else if (splittedFullName.length < 2 || splittedFullName[1] == "") {
      object.invalid_fields.full_name = {
        reason: "no last name provided",
        message:
          "The 'full_name' field must have at least two separated names. Eg: Luke Skywalker",
      };
    } else {
  ...
  if (required.includes("birthday")) {
    if (!birthday) {
      ...
      object.invalid_fields.birthday = {
        reason: "invalid format",
        message:
          "The 'birthday' field must contain a timestamp (interger number).",
      };
    } else {
  ...
  if (required.includes("profile_img")) {
    if (!profile_img) {
      ...
      object.invalid_fields.profile_img = {
        reason: "invalid format",
        message:
          "The 'profile_img' field must have to be a URL. Eg: http://example.com/image.jpg",
      };
    } else {
